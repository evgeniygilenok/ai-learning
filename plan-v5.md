# AI Engineering Roadmap v5

Current as of 2026-07-02.

This version keeps the stage structure, timeline, and job-landing focus of `plan-v4.md` unchanged. The improvement is that the Agentic Workflow Engineering Practice course (lessons 0042-0049) is now a fully integrated, evidence-grade part of the plan instead of a loosely attached outline.

The core principle is unchanged: build one cumulative AI product system that proves you can ship, measure, secure, operate, and explain AI features. Do not collect disconnected demos.

## What v5 Fixes From v4

`plan-v4.md` added the specialization course but left it weaker than the rest of the roadmap:

- Course lessons had no external citations, and their only cited source (a chat attachment) was unrecoverable. Now: every course lesson cites public sources tracked in `RESOURCES.md`, and `sources/agentic-workflow-masterclass-outline.md` preserves the reconstructed outline.
- Course build assignments were paper exercises. Now: each lesson produces a named, committed artifact, and lessons 0043-0045 require running real harness assets, headless one-shot scripts, and strategy comparisons.
- The course introduced ~15 terms with no reference layer. Now: `reference/agentic-workflow-reference.html` is the printable cheat sheet, and the glossary covers the new practice terms.
- The course page listed job proof that did not match this plan's artifact list. Now: both point at the same files (see the course section below).
- Lesson drills were too easy to be feedback. Now: drills use plausible distractors with per-answer explanations, plus a "try it cold, then reveal" exercise per lesson.

## What v4 Fixed From v3

`plan-v3.md` was strong on technical sequence. Its main gaps were around employability and proof.
...
External signals used for this calibration (unchanged since v4):
...
This course is based on the agentic workflow masterclass outline (preserved in `sources/agentic-workflow-masterclass-outline.md`) and should be treated as a deeper practice track, not as extra prerequisite material for every AI engineering role.
...
Time budget: roughly 12-18 hours total (8 lessons at ~40 minutes each plus studio work). At the recommended 8-10 hours/week pace, plan 1.5-2 weeks; this is already inside the "with agent runtime specialization" column of the time budget table above.

Course page and reference:

- `courses/agentic-workflow-engineering.html`
- `reference/agentic-workflow-reference.html` (printable cheat sheet: block contracts, gate types, strategy selection, review packets, WAL entries, rollout memos)

Lessons, each producing a committed artifact:

- 0042: Workflow Blocks And Quality Gates -> `docs/workflow-graph.md`
- 0043: Harness Skills, Hooks, And Subagents -> a real skill file (for example `.cursor/skills/<name>/SKILL.md`) plus notes in `docs/agentic-workflow-course-notes.md`
- 0044: Scriptable Agent Blocks -> a committed headless one-shot script and validator with example outputs
- 0045: Agent Implementation Strategies -> a direct-vs-verification-loop comparison table in `docs/agentic-workflow-course-notes.md`
- 0046: Agentic Planning And Definition Of Done -> `docs/plans/<feature>-plan.md`
- 0047: AI-Assisted Code Review System -> `docs/workflow-review-checklist.md`, run against one real past diff
- 0048: Workflow Continuous Improvement Loop -> `docs/workflow-retrospective.md` plus one landed eval case or gate
- 0049: Team Agent Workflow Rollout -> `docs/workflow-rollout-memo.md`
...
Sequencing note: implementation strategies (0045) intentionally precede planning (0046), following the source outline; feeling the cost of under-planned implementation is what makes the planning discipline land. Plan-first order also works.

Knowledge sources (tracked in `RESOURCES.md`): Anthropic's Building effective agents, Effective context engineering, and Writing tools for agents; Claude Code docs on skills, hooks, and subagents; Cursor headless CLI and output format docs; Google's code review eng-practices; Retromat for retrospective formats.
...
- One harness skill and one scriptable block exist in the repo and have been run for real.
...
- `docs/workflow-rollout-memo.md`.
- A committed harness skill and a scriptable one-shot block with validator.
...
Added in v4. It exists because employers trust shipped proof more than local-only proof.
