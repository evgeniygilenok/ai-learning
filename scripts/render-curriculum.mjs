...
function navigationFor(lesson, index) {
  ...
  const overrides = {
    ...
    "0035": { previous: "0034", next: "0050", alternate: "0042" },
    "0039": { previous: "0030", next: "0040", alternate: "0031" },
    "0040": { previous: "0039", next: null, alternate: "0041" },
    "0049": { previous: "0048", next: null },
    "0050": { previous: "0035", next: "0051" },
    "0051": { previous: "0050", next: "0052" },
    "0052": { previous: "0051", next: "0053" },
    "0053": { previous: "0052", next: "0039", alternate: "0034" },
  };
...
function renderDepthBrief(id) {
  const briefs = {
    ...
    "0050": ["Framework value is measured at the recovery boundary", "Rebuild the same graph, tool, approval, and idempotency contracts rather than inventing a second demo. Use a Postgres checkpointer and stable thread ID, stop at a dynamic interrupt, terminate the process, then resume. Compare hand-rolled and LangGraph behavior on persistence, interrupts, recovery, observability, adoption cost, and failure clarity."],
    "0051": ["MCP standardizes transport, not authority", "Expose the smallest useful schemas and keep actor, tenant, scopes, approval, destinations, and idempotency in trusted server/runtime state. Connect through a real stdio client, then test tenant mismatch, missing scope, replay, and model-supplied attempts to widen authority. Record denied and executed calls without storing secrets."],
    "0052": ["A hosted trace must change a release decision", "Export redacted spans using one dated OpenTelemetry GenAI schema and one hosted OTLP receiver. Port an existing agent case instead of making a showcase-only dataset. Verify the backend can locate a failed real run by run ID, explain its path and usage, and block a known approval or tool regression."],
    "0053": ["Context is a per-step budget with a quality cost", "Assemble only required policy, relevant facts, recent turns, and a provenance-preserving summary. Force a run beyond the uncompressed step budget, then compare tokens, latency, task success, and failure slices before and after compaction on the same cases. Saving tokens is not a win if critical state disappears."],
  };
...
function renderIndex() {
  const stageCards = stages.map((stage) => {
    const required = stage.lessons.map((id) => lessonCard(lessonById[id], lessonById[id].seniorReview ? "senior diff review" : lessonById[id].status)).join("\n");
...
function renderCourse() {
  ...
  <header><a class="back" href="../index.html">← Curriculum index</a><p class="eyebrow">Main course · last reviewed ${reviewed}</p><h1>Applied AI Engineering: from real model call to public production proof.</h1><p class="lede">A self-paced course for an experienced TypeScript/product engineer. Build one regulated-domain, model-driven support system in stages, test failure as deliberately as success, and finish with inspectable public job-ready evidence.</p></header>
  ...
  <section><h2>How the path works</h2><div class="grid"><article class="card"><h3>Core and sequential</h3><p>Stages 1-6 and 8 form the job-ready spine. Complete their gates in order.</p></article><article class="card"><h3>Real after Stage 1</h3><p>The fake provider remains for deterministic tests. Every Stage 2+ core gate also requires a redacted real-model run and trace.</p></article><article class="card"><h3>Supporting and parallel</h3><p>Use quantitative/provider tracks where the project needs them; run orientation and career proof alongside the build.</p></article><article class="card"><h3>Advanced specialization</h3><p>Agent runtime, real-agent stack, and workflow engineering deepen agent-heavy roles after their prerequisite core gates.</p></article></div></section>
  <section id="experienced-agentic-track"><h2>Experienced-engineer agentic track · 10-12 weeks at 8-10 hours/week</h2><div class="callout"><p><strong>Use this overlay if you already have senior product/backend fundamentals.</strong> It preserves every gate and lesson URL while bringing real agent work forward.</p><ol><li><strong>Week 1:</strong> orientation (0036) from 15-20 live postings, then foundations 0001-0005 compressed around the real-provider graduation.</li><li><strong>Weeks 2-3:</strong> controlled workflows 0010-0011, then agent runtime 0031-0035 with real model proposals and approval evidence.</li><li><strong>Weeks 4-6:</strong> real-world agent stack 0050-0053: LangGraph/Postgres, MCP, hosted OTLP evals, and context compaction.</li><li><strong>Weeks 7-8:</strong> agent evaluation 0025, 0034, and 0037; fill retrieval/security gaps needed by the flagship.</li><li><strong>Weeks 9-10:</strong> public portfolio gate 0026-0030 and deployment 0039-0040.</li><li><strong>Weeks 11-12 buffer:</strong> posting-driven gaps, interview drills, and feedback-driven improvement.</li></ol><p><strong>Senior diff reviews:</strong> lessons 0013, 0018, and 0020 are not generic rebuilds. Keep their gates, but document only what model uncertainty, tenant-aware retrieval/tools, provider limits, or AI observability changes from systems you already know.</p></div></section>
  ...
  <section><h2>Common evidence bundle</h2><div class="grid"><article class="card"><h3>Working behavior</h3><p>A reproducible command, UI path, test, or demo. After Stage 1 this means deterministic tests <em>and</em> at least one redacted real-model run/trace.</p></article><article class="card"><h3>Failed run</h3><p>A preserved real or injected failure, diagnosis from trace/eval evidence, and the resulting change.</p></article><article class="card"><h3>Quality result</h3><p>An eval or structured review with representative cases, slices, and stated uncertainty.</p></article><article class="card"><h3>Operational observation</h3><p>Latency, tokens/cost, rate-limit, queue, SLO, security, rollout, or user-feedback evidence.</p></article><article class="card"><h3>Decision record</h3><p>A concise tradeoff, alternatives considered, evidence, decision, and revisit trigger.</p></article></div></section>
  ...
  <section><h2>Default 14-18 week pacing</h2><p>Weeks 1-2: orientation and foundations. Weeks 3-4: grounded retrieval. Week 5: controlled workflows. Weeks 6-8: production boundaries. Weeks 9-10: enterprise hardening. Weeks 11-12: portfolio gate. Weeks 13-14: deployment and feedback. Weeks 15-18 are specialization or buffer. Use the 10-12 week agentic track above when senior engineering fundamentals make the diff-review treatment appropriate.</p></section>
  ...
  <section id="completion"><h2>Final completion gate</h2><div class="callout"><ul><li>All core lesson contracts and stage gates are complete; every Stage 2+ bundle links deterministic evidence plus a real-model trace.</li><li>The flagship workflow is driven by real model tool proposals through application-owned policy, approval, and idempotency gates.</li><li>A clean checkout can run representative tests; an API-key checkout can run real triage and agent paths.</li><li>The learner has a LangGraph/Postgres build with a level-3+ decision record, a working least-scope MCP server, a hosted OTLP eval gate, and measured context compaction.</li><li>The public proof package includes repo, case-study README, hosted/auth-gated demo, 3-minute video, and one technical deep dive.</li><li>The role-proof matrix is derived from 15-20 dated live postings, and every row cites a source.</li><li>The learner can show a failed real run, diagnose it, change the system, and explain the measured result.</li></ul></div></section>
